...
if(isset($_POST['prenom'],$_POST['mdp'],$_POST['email'],$_POST['num_etu'])){//l'utilisateur à cliqué sur "S'inscrire", on demande donc si les champs sont défini avec "isset"
    if(empty($_POST['prenom'])){//le champ pseudo est vide, on arrête l'exécution du script et on affiche un message d'erreur
        ...
        echo "Le Pseudo est trop long, il dépasse 20 caractères.";
    } elseif(empty($_POST['mdp'])){//le champ mot de passe est vide
        echo "Le champ Mot de passe est vide.";
    } elseif(mysqli_num_rows(mysqli_query($mysqli,"SELECT * FROM personne WHERE email='".$_POST['email']."'"))==1){//on vérifie que ce pseudo n'est pas déjà utilisé par un autre membre
        echo "Ce pseudo est déjà utilisé.";
    } elseif(mysqli_num_rows(mysqli_query($mysqli,"SELECT * FROM personne WHERE num_etu='".$_POST['num_etu']."'"))==1){//on vérifie que ce numéro d'étudiant n'est pas déjà utilisé par un autre membre
        echo "Ce numéro d'étudiant est déjà utilisé.";
    } else {
        //toutes les vérifications sont faites, on passe à l'enregistrement dans la base de données:
        //Bien évidement il s'agit là d'un script simplifié au maximum, libre à vous de rajouter des conditions avant l'enregistrement comme la longueur minimum du mot de passe par exemple
        if(!mysqli_query($mysqli,"INSERT INTO personne SET prenom='".$_POST['prenom']."', mdp='".md5($_POST['mdp'])."', num_etu='".$_POST['num_etu']."', email='".$_POST['email']."'")){//on crypte le mot de passe avec la fonction propre à PHP: md5()
